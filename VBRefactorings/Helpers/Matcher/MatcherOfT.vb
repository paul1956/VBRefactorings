...
Partial Friend MustInherit Class Matcher(Of T)
    ...
    Public MustOverride Function TryMatch(sequence As IList(Of T), ByRef index As Integer) As Boolean

    Friend Shared Function [Single](predicate As Func(Of T, Boolean), description As String) As Matcher(Of T)
        Return New SingleMatcher(predicate, description)
    End Function

    Friend Shared Function Choice(matcher1 As Matcher(Of T), matcher2 As Matcher(Of T)) As Matcher(Of T)
        Return New ChoiceMatcher(matcher1, matcher2)
    End Function

    Friend Shared Function OneOrMore(M As Matcher(Of T)) As Matcher(Of T)
        ' m+ is the same as (m m*)
        Return Sequence(M, Repeat(M))
    End Function

    Friend Shared Function Repeat(M As Matcher(Of T)) As Matcher(Of T)
        Return New RepeatMatcher(M)
    End Function

    Friend Shared Function Sequence(ParamArray matchers() As Matcher(Of T)) As Matcher(Of T)
